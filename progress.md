...
## 2026-09-21：OMP Web 界面调整

- 用户要求仅处理前端 UI 与功能，避免读取无关的后端和环境信息。
- 已确认 `AppShell` 与 `AppShell-layout` 已实现桌面侧栏宽度拖拽和持久化；本轮将复用该基础，调整顶栏、模型设置入口和菜单位置。
- 已完成：顶栏左侧保留侧栏切换，新增返回/前进；原有 Ellipsis 溢出浮窗已移除。
- 已完成：侧栏标题区新增可展开的“菜单”，其中保留主题、语言、完整历史、分支和系统提示词入口。
- 已完成：模型选择器不再展示“添加提供商/模型设置”快捷入口；模型与 API 配置继续在“设置 → API Keys & Providers”中管理。
- 验证：`npm.cmd run typecheck` 与 `npm.cmd run lint` 均通过；针对模型选择器的新断言通过。完整 `ChatInput.test.mjs` 仍有 10 项既有本地化文案断言失败，与本次功能改动无关。

## 2026-09-21：AcadMate 实际界面重构

- 已确认用户实际使用的是 `/academate`，而不是旧的 OMP 工作区；根路径 `/` 已重定向到 AcadMate，界面内不再提供返回 OMP 的链接。
- 已在 `AcadMateWorkspace` 实现 GPT Windows 风格顶栏：切换侧边栏、返回、前进；顶栏不含菜单栏和模型设置按钮。
- AcadMate 左侧栏现在可以从右缘拖拽到 220–520px，双击分隔条恢复默认 280px，宽度会保存在浏览器中。
- AcadMate 侧栏顶部新增点击展开的“菜单”；设置和退出操作在该浮层中，模型/API 表单从这里打开。
- AcadMate 会话列表的每一项右侧新增垃圾桶按钮；删除采用明确确认、后端 `DELETE` 代理和删除后自动选择剩余会话的流程，不使用归档。
- 删除初次返回 404 后已确认根因：AcadMate Express 原先没有会话删除路由。现已在 `Code/server/routes/conversations.ts` 添加按当前用户限制的永久删除路由；服务端类型检查通过，Express 健康检查返回 200。
